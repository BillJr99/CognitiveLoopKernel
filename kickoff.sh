...
# the agents' point of view. The launcher (scripts/clk) and the
...
# Convenience shim at the project root so users still type
# ``./scripts/clk`` if they prefer; the actual implementation lives
# under .clk/harness/. The shim exports CLK_PROJECT_ROOT so the harness
# launcher resolves harness state in the right place even though the
# script itself lives several levels deep under .clk/.
mkdir -p "$KICKOFF_DIR/scripts"
cat > "$KICKOFF_DIR/scripts/clk" <<'SHIM'
#!/usr/bin/env bash
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
...
# Write a kickoff-specific .gitignore. Agents now operate at the project
# root directly (no workspace/ indirection) so the project tree IS the
# deliverable. The harness lives entirely under .clk/ and is gitignored
# except for curated state files that constitute project memory.
cat > .gitignore <<'GITIGNORE'
# CLK launcher shim (regenerated by kickoff).
/scripts/clk
/.env
/.env.example

# CLK harness state. Project memory worth tracking lives in
# .clk/state/{idea.json,system_brief.md,prd.json,decisions.md,progress.md,
# casting.log,done.md,blackboard summaries} - the rest is local cache /
# runtime artifacts. Harness sources live in .clk/harness/ and are
# refreshed by re-running kickoff, so they're not tracked either.
.clk/harness/
.clk/venv/
.clk/node/
.clk/tools/
