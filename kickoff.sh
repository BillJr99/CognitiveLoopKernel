...
mkdir -p .clk
# Workspace is the agents' filesystem root: ACTION blocks resolve here, run
# commands cwd here. Keeping it separate from the harness sources means git
# history shows the project being built, not the harness scaffolding.
mkdir -p workspace

# Write a kickoff-specific .gitignore that hides the harness sources copied
# in from the parent repo, plus the harness's own state directory. What's
# tracked: workspace/ (the actual project being built), KICKOFF.md, and
# the curated state files in .clk/state/ that constitute project memory.
cat > .gitignore <<'GITIGNORE'
# CLK harness sources (copied from the source repo on kickoff). These are
# implementation, not deliverable - re-run kickoff to refresh.
/clk_harness/
/scripts/
/kickoff.sh
/.env
...
# casting.log,done.md} - the rest is local cache / runtime artifacts.
.clk/venv/
.clk/node/
.clk/tools/
.clk/logs/
