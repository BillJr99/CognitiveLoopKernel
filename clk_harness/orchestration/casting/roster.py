"""Roster registry: role/agent definitions and mutation.

The immutable BASELINE_AGENTS, the duplicate-detection machinery
(name keys, synonym table, prompt-body similarity), prompt-file
scaffolding, and the registry mutators (``register_role``,
``remove_role``, ``write_workflow``).
"""
...
if TYPE_CHECKING:
    from .director import RoleProposal, WorkflowProposal
